...
class TransactionMonitor:
    def __init__(self):
        ...
        self.kafka_thread_frauds = KafkaConsumerThread(
            "Frauds", self.message_queue)

        self.kafka_thread_transactions.start()
        self.kafka_thread_frauds.start()

        self.fig, (self.ax1, self.ax2, self.table) = plt.subplots(
            1, 3, figsize=(12, 8))
    ...
    def update_table(self):
        ...
        for transaction_type in self.transaction_counts:
            ...
            is_valid = self.validate_transaction(
                transaction_type, generated, detected)
            table_data.append(
                [transaction_type, generated, detected, is_valid])

        if not table_data:  # Check if table_data is empty
            return  # Skip creating the table if there is no data

        self.table.clear()
        self.table.axis('off')
        self.table.table(cellText=table_data, colLabels=[
                         'Type', 'Generated', 'Detected', 'IsValid'], loc='center', cellLoc='center')

        table = self.table.table(cellText=table_data, colLabels=[
                                 'Type', 'Generated', 'Detected', 'IsValid'], loc='center', cellLoc='center')
    ...
    def update_plots(self, _):
        ...
        self.plot_transactions(self.ax2, self.fraud_counts,
    ...
    def plot_transactions(self, ax, counts, title):
        ...
        # Assign colors based on hash of transaction type
        bar_colors = [self.colors[hash(t) % len(self.colors)]
