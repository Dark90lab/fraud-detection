...
class UserGenerator(BaseGenerator):
    def __init__(self, config_provider: ConfigProvider):
        ...
        self.generated_cards_ids: Dict[int, list[int]] = dict()
    ...
    def get_user_limit(self):
        return round(np.random.uniform(self.config.MIN_FOUNDS, self.config.MAX_FOUNDS), 2)

    def get_user_data(self) -> User:
        user_id = random.randint(1, self.config.USERS_NUMBER)

        if self.generated_cards_ids.get(user_id) is None:
            ...
            self.generated_cards_ids[user_id] = list(cards_assigned)

        return User(user_id, self.generated_cards_ids[user_id], self.get_user_limit())
